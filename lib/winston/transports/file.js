...
var File = exports.File = function (options) {
  ...
  this.fileDroppingHandler = ( options.maxsize && options.maxFiles && options.fileDroppingHandler ) ? options.fileDroppingHandler : null;
...
File.prototype._getFile = function (inc) {
  ...
  if (inc) {
    ...
    //
    // Check for maxFiles option and deal with the files to be dropped.
    if (this.maxFiles && (this._created >= (this.maxFiles - 1))) {
      remaining = this._created - (this.maxFiles - 1);

      var fileToDelete = path.join( this.dirname, basename + ( remaining===0 ? '' : remaining ) + ext );

      if( self.fileDroppingHandler ) {
        // Handle the file to be dropped by the custom handler.
        fileDroppingHandler( fileToDelete );
      }
      else {
        // Remove the file.
